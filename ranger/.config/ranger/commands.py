...
class fzf_rg_contents_open(Command):
    """
    :fzf_content_open
    ...
    def execute(self):
        ...
        if rg is not None and fzf is not None and bat is not None and editor is not None:
            ...
            #        'rg --line-number "${1:-.}" | fzf --delimiter \':\' --preview \'bat --color=always --highlight-line {2} {1}\' | awk -F \':\' \'{print "+"$2" "$1}\'',
            #    universal_newlines=True,stdout=subprocess.PIPE)
            #print(fzf.args)
            # fzf --preview spawns the other window, since rg prints linenum, you delimit it at : meaning that the rg output is filename:num:data preview will have {1} be the filename and {2}
            # will be line num, so you're highlighting the linenum in bat, and opening that file
            # I just had to fix the arg substitution
            fzf = self.fm.execute_command(
                    'rg --line-number ' + self.arg(1) + ' | fzf --delimiter \':\' --preview \'bat --color=always --highlight-line {2} {1}\' | awk -F \':\' \'{print "+"$2" "$1}\'',
                universal_newlines=True,stdout=subprocess.PIPE)
